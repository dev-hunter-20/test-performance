...
const TableListApp = (props) => {
  ...
  return (
    <Row key={index} className="item-detail">
      ...
      <Col span={2} style={{ fontSize: "15px", alignItems: "center", display: "flex" }}>{item ? renderTagType(item.categories.reduce((sum, item) => sum + item.count, 0)) : -''}</Col>
      ...
      <Col span={2} className="flex items-center justify-center">
        <div>{itemChild.review_count > 0 ? itemChild.review_count : itemChild.review || null}</div>
      </Col>
    </Row>
  );
};
